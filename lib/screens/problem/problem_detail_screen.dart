...
class _ProblemDetailScreenState extends State<ProblemDetailScreen>
    ...
    return Scaffold(
      ...
      body: Consumer<ProblemProvider>(
        builder: (context, problemProvider, _) {
          if (problemProvider.isLoading && problemProvider.selectedProblem == null) {
            return _buildLoadingState(colors);
          } else if (problemProvider.errorMessage.isNotEmpty && problemProvider.selectedProblem == null) {
            return _buildErrorState(problemProvider.errorMessage, colors);
          } else if (problemProvider.selectedProblem == null) {
            return _buildErrorState(
              problemProvider.errorMessage.isNotEmpty ? problemProvider.errorMessage : 'Problème non trouvé',
              colors,
            );
          } else {
            final problem = problemProvider.selectedProblem!;
            final String? photoUrl = problem['photo_url'] as String?;
            return _buildProblemDetailContent(problem, photoUrl, theme, safePadding, screenWidth);
          }
        },
      ),
    );
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            ...
            style: GoogleFonts.inter(color: colors.onSurfaceVariant, fontSize: 16),
          )
        ],
      ),
    ).animate().fadeIn();
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              'Oups! Erreur',
              style: GoogleFonts.inter(color: colors.error, fontSize: 20, fontWeight: FontWeight.bold),
            ),
            const SizedBox(height: 10),
            Text(
              message,
              textAlign: TextAlign.center,
              style: GoogleFonts.inter(color: colors.onSurfaceVariant, fontSize: 15, height: 1.4),
            ),
  ...
  Widget _buildProblemDetailContent(Map<String, dynamic> problem, String? photoUrl, ThemeData theme, EdgeInsets safePadding, double screenWidth) {
    final ColorScheme colors = theme.colorScheme;
    ...
    try {
      ...
      formattedCreatedDate = DateFormat('dd MMMM yyyy, HH:mm', 'fr_FR').format(date);
    } catch (e) { print("Error parsing date: $e"); }

    // Status Configuration
    final statusConfig = _getStatusConfig(problem['status'], colors);
    final statusLabel = statusConfig['label'] ?? problem['status_display'] ?? 'Inconnu';
      ...
      return AnimatedBuilder(
        ...
        builder: (context, _) {
          final animationValue = Curves.easeOut.transform(
            ((_staggerController.value - intervalStart) / (intervalEnd - intervalStart)).clamp(0.0, 1.0)
          );
          return Opacity(
    ...
    return CustomScrollView(
      ...
      slivers: [
        SliverAppBar(
          pinned: true,
          stretch: true,
          expandedHeight: photoUrl != null && photoUrl.isNotEmpty ? screenWidth * 0.7 : kToolbarHeight + safePadding.top,
            ...
            delegate: SliverChildListDelegate([
              ...
              animateItem(Row(
                ...
                children: [
                  Chip(
                    ...
                    label: Text(statusLabel, style: GoogleFonts.inter(fontWeight: FontWeight.w600, color: colors.onPrimary)),
              ...
              if (problem['category'] != null)
                animateItem(Align(
                  ...
                  child: Chip(
                    avatar: Icon(Icons.category_rounded, size: 18, color: colors.secondary),
                    label: Text(
                      problem['category']['name'] ?? 'Catégorie inconnue',
                      style: GoogleFonts.inter(fontWeight: FontWeight.w500, color: colors.onSecondaryContainer),
                    ),
              ...
              // Description Section
              animateItem(_buildSectionTitle('Description', Icons.description_outlined, theme)),
              const SizedBox(height: 10),
              animateItem(_buildInfoCard(
                child: Text(
                  problem['description'] ?? 'Pas de description fournie.',
                  style: GoogleFonts.inter(fontSize: 15, color: colors.onSurface, height: 1.5),
                ),
                theme: theme,
              )),
              ...
              if (voiceRecordUrl != null && voiceRecordUrl.isNotEmpty)
                ...[
                  animateItem(_buildSectionTitle('Note Vocale', Icons.graphic_eq_rounded, theme)),
                  const SizedBox(height: 10),
                  animateItem(_buildInfoCard(
                    child: _buildAudioPlayerWidget(theme),
                    theme: theme,
                  )),
                  const SizedBox(height: 28),
                ],
              ...
              // Location Section
              animateItem(_buildSectionTitle('Emplacement', Icons.location_on_outlined, theme)),
              const SizedBox(height: 10),
              animateItem(_buildInfoCard(
                child: Column(
                  ...
                  children: [
                    if (problem['municipality'] != null)
                      Text(
                        problem['municipality']['name'] ?? 'Municipalité inconnue',
                        style: GoogleFonts.inter(fontSize: 16, fontWeight: FontWeight.w500, color: colors.onSurface),
                      ),
                    if (problem['municipality'] != null && _problemLocationLatLng != null)
                    ...
                    if (_problemLocationLatLng != null)
                      ...
                      Padding(
                        ...
                        child: Text(
                          ...
                          style: GoogleFonts.inter(color: colors.onSurfaceVariant.withOpacity(0.7)),
                        ),
                      ),
                  ],
                ),
              ...
              if (videoUrl != null && videoUrl.isNotEmpty)
                ...[
                  animateItem(_buildSectionTitle('Vidéo', Icons.videocam_rounded, theme)),
                  const SizedBox(height: 10),
                  animateItem(_buildInfoCard(
                    child: _buildVideoPlayerWidget(theme),
                    theme: theme,
                  )),
                  const SizedBox(height: 28),
                ],
              ...
              if (documentUrl != null && documentUrl.isNotEmpty)
                ...[
                  animateItem(_buildSectionTitle('Document', Icons.attach_file_rounded, theme)),
                  const SizedBox(height: 10),
                  animateItem(_buildInfoCard(
                    child: _buildDocumentTile(documentName ?? 'Document', documentUrl, theme),
                    theme: theme,
                  )),
                  const SizedBox(height: 28),
                ],
              ...
              if (problem['comment'] != null && problem['comment'].toString().isNotEmpty)
                ...[
                  animateItem(_buildSectionTitle('Commentaire Admin', Icons.admin_panel_settings_rounded, theme)),
                  const SizedBox(height: 10),
                  animateItem(_buildInfoCard(
                    child: Text(
                      problem['comment'],
                      style: GoogleFonts.inter(fontSize: 15, color: colors.onPrimaryContainer, height: 1.5),
                    ),
                    theme: theme,
                    isHighlighted: true,
                  )),
  ...
  Widget _buildInfoCard({required Widget child, required ThemeData theme, bool isHighlighted = false}) {
    final ColorScheme colors = theme.colorScheme;
    return Container(
        ...
        children: [
          Image.network(
            ...
            loadingBuilder: (context, child, loadingProgress) {
              if (loadingProgress == null) return child.animate().fadeIn(duration: 300.ms);
              return Center(
            ...
            child: Stack(
              children: [
                FlutterMap(
                  ...
                  options: MapOptions(
                    initialCenter: _problemLocationLatLng!, // Assumes not null here
                    initialZoom: 15.5,
                    minZoom: 5.0,
                    maxZoom: 18.0,
                  ...
                  children: [
                    TileLayer(
                      ...
                      userAgentPackageName: 'com.example.citoyen_app', // Replace with your actual package name
                    ...
                    MarkerLayer(
                      markers: [
                        Marker(
                          ...
                          width: 60,
                          height: 60,
                          alignment: Alignment.topCenter, // Adjust alignment for pin shape
                          child: Icon(
                            Icons.location_pin,
                            color: colors.error,
                            size: 50,
                ...
                Positioned(
                  ...
                  child: FloatingActionButton.small(
                    ...
                    elevation: 4,
                    tooltip: _isMapExpanded ? 'Réduire la carte' : 'Agrandir la carte',
                    ...
                    child: AnimatedSwitcher(
                      ...
                      child: Icon(
                        ...
                        key: ValueKey<bool>(_isMapExpanded), // Key for animation
                      ),
                    ),
                  ),
                ),
  ...
  Widget _buildVideoPlayerWidget(ThemeData theme) {
    final ColorScheme colors = theme.colorScheme;
    if (_isVideoLoading) {
    ...
    if (_videoController == null || !_videoController!.value.isInitialized) {
      return Row(children: [
        ...
        const Text("Vidéo non disponible ou erreur")
      ]);
    }
    return Column(
      children: [
  ...
  Widget _buildAudioPlayerWidget(ThemeData theme) {
    final ColorScheme colors = theme.colorScheme;
    if (_isAudioLoading) {
      return const Center(child: Padding(
        padding: EdgeInsets.symmetric(vertical: 20.0),
    ...
    if (_audioPlayer == null) {
      return Row(children: [
        Icon(Icons.error_outline_rounded, color: theme.colorScheme.error, size: 20),
        const SizedBox(width: 8),
        const Text("Audio non disponible ou erreur")
      ]);
    }
    return Row(
      children: [
        ...
        IconButton(
          icon: AnimatedSwitcher(
            ...
            transitionBuilder: (child, animation) => ScaleTransition(scale: animation, child: child),
            child: Icon(
              _isAudioPlaying ? Icons.pause_circle_filled_rounded : Icons.play_circle_filled_rounded,
              key: ValueKey<bool>(_isAudioPlaying), // Key for animation
              color: theme.colorScheme.primary,
              size: 40,
            ),
          ),
          tooltip: _isAudioPlaying ? 'Pause' : 'Lecture',
          onPressed: () {
            if (_isAudioPlaying) {
              _audioPlayer!.pause();
            } else {
            ...
            children: [
              SliderTheme(
                 ...
                 child: Slider(
                   ...
                   min: 0.0,
                   max: _audioDuration.inSeconds.toDouble() > 0 ? _audioDuration.inSeconds.toDouble() : 1.0, // Avoid max <= min
                   onChanged: (value) {
              ...
              Padding(
                ...
                child: Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    Text(_formatDuration(_audioPosition), style: GoogleFonts.inter(fontSize: 12, color: theme.colorScheme.onSurfaceVariant)),
                    Text(_formatDuration(_audioDuration), style: GoogleFonts.inter(fontSize: 12, color: theme.colorScheme.onSurfaceVariant)),
                  ],
                ),
              ),
            ],
        ...
        child: Padding(
          ...
          child: Row(
            children: [
              ...
              const SizedBox(width: 8),
              Icon(Icons.download_for_offline_rounded, color: theme.colorScheme.onSurfaceVariant.withOpacity(0.7), size: 22),
            ],
          ),
        ),
      ),
