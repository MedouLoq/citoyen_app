...
import 'package:citoyen_app/screens/auth/login_screen.dart';
import 'package:citoyen_app/screens/auth/register_screen.dart';
...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer<DashboardProvider>(builder: (context, dashboardProvider, child) {
      return Scaffold(
        ...
        appBar: AppBar(
          title: Text('Tableau de Bord', style: GoogleFonts.inter(fontWeight: FontWeight.w600)),
          automaticallyImplyLeading: false, // No back button on main dashboard
          actions: [
            IconButton(
              icon: Icon(Icons.notifications_none_outlined, color: colors.primary),
              onPressed: () {
  ...
  MaterialPageRoute(builder: (context) => NotificationsScreen()),
        ...
        floatingActionButton: _currentIndex == 0 // Show FAB only on Dashboard Home Tab
          ...
          label: Text('Signaler', style: GoogleFonts.inter(fontWeight: FontWeight.w600, color: colors.onPrimary)),
        )
            .animate()
