...
class _AuthScreenState extends State<AuthScreen> with TickerProviderStateMixin {
  ...
  Future<void> _login() async {
    if (_loginFormKey.currentState!.validate()) {
      ...
      const String apiUrl = 'http://192.168.66.228:8000/api/login/';
  ...
  Future<bool> _sendVerificationCode(
    ...
    try {
      final response = await http
          .post(
            Uri.parse('http://192.168.66.228:8000/api/send-code/'),
            headers: {'Content-Type': 'application/json'},
            body: jsonEncode({
  ...
  Future<void> _register() async {
    if (_registerFormKey.currentState!.validate() &&
      ...
      const String apiUrl = 'http://192.168.66.228:8000/api/register/';
              ...
              items: (String filter, _) async {
                try {
                  final res = await http
                      .get(Uri.parse(
                          'http://192.168.66.228:8000/api/municipalities/'))
